      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {instructorsData.map((inst) => (
            <div
              key={inst.name}
              className="relative rounded-lg overflow-hidden shadow-lg m-2 group
                         transform-gpu transition duration-500
                         sm:hover:scale-105 sm:hover:-rotate-1"
            >
              <img
                src={inst.img}
                ...
                <div className="instructor-card-content w-full text-left">
                  <div
                    className="transition-all duration-500
                               text-white
                               sm:opacity-100 sm:translate-y-20
                               sm:group-hover:opacity-100 sm:group-hover:translate-y-0"
                  >
                    <h3 className="text-2xl font-bold">{inst.name}</h3>
                    <p className="text-blue-400 font-semibold">{inst.title}</p>
                    <p className="transition-all duration-500 text-gray-300 text-sm mt-1
                               sm:opacity-0 sm:translate-y-20
                               sm:group-hover:opacity-100 sm:group-hover:translate-y-0">{inst.bio}</p>
                  </div>
                </div>
              </div>
            </div>
